...
df = pd.read_csv("data/reviews/reviews_to_approved.csv")
df2 = pd.read_csv("data/reviews/approved_reviews.csv")
...
def add_review(df, station, review_text, score=None):
    ...
    df.to_csv("data/reviews/reviews_to_approved.csv",index=False)
...
def approve_review(df,df2, review_id):
    ...
    # save chabges
    df2.to_csv("data/reviews/approved_reviews.csv", index=False)
